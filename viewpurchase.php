...
<?php
	...
	$sql = "SELECT COUNT(*) FROM cart , product, users WHERE cart.cust_id = $userid AND users.user_id = $userid AND cart.checkout = 'y' AND product.id = cart.product_id";
	$query = (mysqli_query($db_conx,$sql));
	...
	$selectproducts = "SELECT * FROM cart , product, users WHERE cart.cust_id = $userid AND users.user_id = $userid AND cart.checkout = 'y' AND product.id = cart.product_id";
	...
	else
	{
		echo "<table align='center' cellspacing='1' cellpadding='0' width='80%'>
				...
				<th align='center' bgcolor='e6e6e6'>PRODUCT ID</th>
		...
		for($loop = 0; $loop < $countrows; $loop++)
		{
			...
			while($row = mysqli_fetch_array($query, MYSQLI_ASSOC))
			{
				...
				$list ="<tr align='center'><td>" . $checked . "</td><td>" . $trans . "</td><td>" . $productid . "</td><td>" . $prodname . "</td><td>" . $qty . "</td>";
