...
<p align="center"><img src="image/stationary_logo.jpg" /></p>
...
<?php
	...
	$sql = "SELECT COUNT(*) FROM cart , product, users WHERE cart.cust_id = $userid AND users.user_id = $userid AND cart.checkout = 'y' AND product.id = cart.product_id AND cart.checkedon = '$today' AND cart.trans = '$code'";
	...
	$selectproducts = "SELECT * FROM cart , product, users WHERE cart.cust_id = $userid AND users.user_id = $userid AND cart.checkout = 'y' AND product.id = cart.product_id AND cart.checkedon = '$today' AND cart.trans = '$code'";
	...
	else
	{
		...
		for($loop = 0; $loop < $countrows; $loop++)
		{
			...
			while($row = mysqli_fetch_array($query, MYSQLI_ASSOC))
			{
				...
				$list ="<tr align='center'><td>" . $productid . "</td><td>" . $prodname . "</td><td>" . $qty . "</td>";
...
<tr>
	...
	<td align="left"><font face=""><b>Total Quantity</b></font></td>
...
<tr>
	<td align="left"><b>Total Amount (Tk)</b></td>
